...
const freqMap = fs
  .readFileSync(path.resolve(__dirname, "../data/phrases.txt"))
  .toString()
  .split("\n")
  .map(line => line.trim())
  .map(line => ({
  ...
  .reduce(
    (map, { word, weight }) => (word ? map.set(word, weight) : map),
    new Map()
  );

const [words, randMax] = fs
  .readFileSync(path.resolve(__dirname, "../data/chosen.txt"))
  .toString()
  .split("\n")
  .map(word => ({ word, weight: freqMap.get(word) }))
  .sort((a, b) => b.weight - a.weight || a.word.localeCompare(b.word))
  .reduce(
    ([arr, total], { word, weight }) => {
